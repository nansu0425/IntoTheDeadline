...
bool SAnimationViewerWindow::Initialize(float StartX, float StartY, float Width, float Height, UWorld* InWorld, ID3D11Device* InDevice)
{
    World = InWorld;
    Device = InDevice;
...
void SAnimationViewerWindow::OpenNewTab(const char* Name)
{
    ViewerState* State = AnimationViewerBootstrap::CreateViewerState(Name, World, Device);
    if (!State) return;
...
void SAnimationViewerWindow::CloseTab(int Index)
{
...
void SAnimationViewerWindow::UpdateBoneTransformFromSkeleton(ViewerState* State)
{
...
void SAnimationViewerWindow::ApplyBoneTransform(ViewerState* State)
{
    if (!State || !State->CurrentMesh || State->SelectedBoneIndex < 0)
        return;
...
void SAnimationViewerWindow::ExpandToSelectedBone(ViewerState* State, int32 BoneIndex)
{
